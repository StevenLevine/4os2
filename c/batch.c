...
int batch( int argc, char **argv )
{
    ...
    // We can not chain to batch from REXX script
    if ( cv.call_flag & CF_REXX_SUBCOMM && ~cv.call_flag & CF_NESTED_BATCH )
        return( error( 1803, NULL ) );
    ...
    if ( cv.call_flag & CF_REXX_SCRIPT )
    ...
    if ( cv.call_flag ) {
        ...
        if ( cv.call_flag & CF_REXX_SCRIPT ) {
            ...
            if ( ( cv.call_flag & CF_CALL_Q ) && (( bframe[ cv.bn ].echo_state & 2 ) == 0 ) )
...
int call_cmd(int argc, char **argv)
{
    ...
    for ( argc = 0; (( arg = ntharg( argv[1], argc )) != NULL ); argc++ ) {
        ...
        if ( _stricmp( arg, "/Q" ) == 0 ) {
            ...
            cv.call_flag = CF_CALL_Q | CF_NESTED_BATCH;
            break;
        }
    }
...
int for_cmd( int argc, char **argv)
{
    ...
    for ( i = 0; (( arg = ntharg( *argv, i )) != NULL ); i++ ) {
        if ( ( *arg == '/' ) && (( arg[1] == '[' ) || ( _ctoupper( arg[1] ) == 'A' )) ) {
            if ( i > 0 )
                strcat( source, " " );
            strcat( source, arg );
        } //else
    ...
    if ( fForFlags & 1 )
        mode |= FIND_BYATTS | FILE_DIRECTORY | FILE_READONLY | FILE_HIDDEN | FILE_SYSTEM; //0x417;
...
int if_cmd( int argc, char **argv )
{
    ...
    // Special values allow outer levels to map return code to 0 when needed
    return(( fIff ) ? IFF_EXPR_FALSE : IF_EXPR_FALSE );
}
...
int shift_cmd( int argc, char **argv )
{
    ...
    // Fail silently if shift/rotate count does not make sense
    if ( argc > 1 ) {
        // Get shift/rotate count, default to 1 (i.e. arg count is 1)
        if ( argv[1][0] == '/' ) {
            // Shift args left one position starting at requested argument and
            // drop argument at start position
            argc = atoi( argv[1] + 1 );
            if ( argc > 0 ) {
                for ( c = 1; bframe[ cv.bn ].Argv[c] != NULL ; c++ )
                    ;                             // Count args
                if ( argc <= c ) {
                    for ( ; argc < c ; argc++ )
                        bframe[ cv.bn ].Argv[argc] = bframe[ cv.bn].Argv[argc+1];
                    bframe[ cv.bn ].Argv[argc] = NULL;    // Chop last
                }
                return 0;
            }
        }
        argc = atoi( argv[1] );             // Set rotate count
    }

    // Rotate args by count
