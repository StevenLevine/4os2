...
static int _list( char *pszFileName )
{
    ...
    for ( ; ; ) {
        ...
        i = sprintf( szHeader, LIST_LINE, LFile.nListHorizOffset, LFile.lCurrentLine + gpIniptr->ListRowStart,
                     (int)((( (long long)LFile.lViewPtr + 1 ) * 100 ) / LFile.lSize ));
        ...
        case LIST_INFO_CHAR:
            ...
            if ((fFSType = ifs_type(LFile.szName)) != FS_FAT)
                i = 11;
            ...
            if ( uListFlags & LIST_STDIN )
                ...
                strcpy( szLine, FormatDate(dir.fdLW.fdateLastWrite.month,dir.fdLW.fdateLastWrite.day,dir.fdLW.fdateLastWrite.year+1980, 0 ) );
                strcpy( szBuf, FormatDate(dir.fdLA.fdateLastAccess.month,dir.fdLA.fdateLastAccess.day,dir.fdLA.fdateLastAccess.year+1980, 0 ) );
                sprintf( fptr,
                         fFSType != FS_FAT ? LIST_INFO_HPFS : LIST_INFO_FAT,
                         LFile.szName,
                         szDescription,
                         dir.size,
                         szLine, // last write date
                         dir.ftLW.ftimeLastWrite.hours,
                         gaCountryInfo.szTimeSeparator[0],
                         dir.ftLW.ftimeLastWrite.minutes,
                         szBuf, // last access date
                         dir.ftLA.ftimeLastAccess.hours,
                         gaCountryInfo.szTimeSeparator[0],
                         dir.ftLA.ftimeLastAccess.minutes,
                         FormatDate(dir.fdC.fdateCreation.month,
                                    dir.fdC.fdateCreation.day,
                                    dir.fdC.fdateCreation.year+1980, 0 ),
                         dir.ftC.ftimeCreation.hours,
                         gaCountryInfo.szTimeSeparator[0],
                         dir.ftC.ftimeCreation.minutes );
