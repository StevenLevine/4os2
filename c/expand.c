...
int CopyFromClipboard( char *pszFilename )
{
    ...
    // Try get the clipboard text.
    rval = GetClipboardText( &ulState, &lpClipMemory );
    if ( rval ) {
    ...
    for ( loopctr = 0; ; loopctr++ ) {
        ...
        *eol = '\0';
        ...
        for ( cptr = aliasline; ( *cptr != '\0' ); ) {
            ...
            if ( cptr[1] == '#' ) {
                ...
                for ( argnum = 1;
                    ntharg( line, argnum | NTHARG_BRACKETS_ARE_QUOTES ) != NULL;
                    argnum++ ) {
                    ;
                }
            ...
            arg = ntharg( line, argnum | NTHARG_BRACKETS_ARE_QUOTES );
        ...
        *eol = eolchar;
        ...
        if ( vars_exist ) {
            ...
            if ( ntharg( line + alen, vars_exist | NTHARG_BRACKETS_ARE_QUOTES ) != NULL &&
                 arg > gpNthptr ) {
                arg = gpNthptr;
...
static int var_func(char *func, char *szBuffer )
{
    ...
    long lOffset = 0L;
    char *fptr;
    char *arg, szDrive[512];		// Holds drive spec, also used as work buffer
    QDISKINFO DiskInfo;
    FILESEARCH dir;
    ...
    // set drive arg (so both "a" and "a:" and path spec will work) 2010-09-10 SHL
    if ( ( fname[0] == '\\' ) && ( fname[1] == '\\' ) )
        strcpy( szDrive, fname );
    else
        sprintf( szDrive, FMT_DISK, isalpha( fname[0] ) && ( fname[1] == '\0' || fname[1] == ':' ) ? *fname : gcdisk( NULL ) + 64 );
                ...
                case FUNC_CLIP:
                    {
                        ...
                        n = -1;
                        *var = '\0';
                        if ( GetClipboardText( &ulState, &lpClipMemory ) != 0 ) {
...
#define M ( FIND_CLOSE_GLOBAL | FILE_DIRECTORY | FILE_READONLY | FILE_HIDDEN | FILE_SYSTEM)    // 0x2017
                                if ( find_file( FIND_FIRST, fname, M, &dir, NULL ) != NULL ) {
#undef M
                        ...
                        if ( offset == FUNC_FILEAGE ) {
                            ...
                            if ( n == 0 ) {
                                if ( *( var = FormatDate( dir.fdLW.fdateLastWrite.month, dir.fdLW.fdateLastWrite.day, dir.fdLW.fdateLastWrite.year+1980, 0 )) == ' ' )
                                    *var = '0';
                            } else if ( n == 1 ) {
                                if ( *( var = FormatDate( dir.fdLA.fdateLastAccess.month, dir.fdLA.fdateLastAccess.day, dir.fdLA.fdateLastAccess.year+1980, 0 )) == ' ' )
                                    *var = '0';
                            } else {
                                if ( *( var = FormatDate( dir.fdC.fdateCreation.month, dir.fdC.fdateCreation.day, dir.fdC.fdateCreation.year+1980, 0 )) == ' ' )
                                    *var = '0';
                            }
                ...
                case FUNC_MAKEAGE:
                    ...
                    month = day = year = 0;
                    rval = GetStrDate( var, &month, &day, &year );

		    // 2012-02-17 SHL Normalize year
		    if ( year < 80 )
			year += 2000;
		    else if ( year < 100 )
			year += 1900;
		...
		case FUNC_MAKEDATE:
		    ...
		    // replace leading space with a 0
		    if ( *( var = FormatDate( month, day, year, 0 )) == ' ' )
			*var = '0';
		    break;
		...
		case FUNC_MASTER:
		    ...
		    // return value of variable in master environment
		    var = get_list( var, glpMasterEnvironment );
		    break;
		...
		case FUNC_READSCR:
			...
			day = 2;
			(void)VioReadCellStr((char *)( var+offset), (VIO_PUTYPE)&day, i,n, 0 );
		    }
		...
		case FUNC_REPLACE:
			...
			// replaces occurrences of "str1" with "str2"
                        ...
                        // var == szBuffer means the 1st parameter is missing (not allowed)
                        if ( ( *var != ',' ) || ( var == szBuffer ) )
			    return ERROR_INVALID_PARAMETER;
		...
		case FUNC_EAWRITE:
		    ...
		    *fptr++ = '\0';
		    var = first_arg( var );
                    ...
                    strip_leading( fname, WHITESPACE );
                    strip_trailing( fname, WHITESPACE );
...
static char * var_internal( char *var )
{
	...
	if ( _stricmp( var + 1, VAR_ARRAY[offset]) == 0 ) {
...
#if defined( IOCTL_POWER ) && defined( POWER_GETPOWERSTATUS )
		...
		case VAR_DATE:
		    ...
		    // replace leading space with a 0
		    if ( *( var = FormatDate(sysDateTime.month,sysDateTime.day,sysDateTime.year, 0 )) == ' ' )
                        *var = '0';
                    break;
		    ...
		    // system date/time
                    ...
                    // replace leading space with a 0
                    if ( *( var = FormatDate(sysDateTime.month,sysDateTime.day,sysDateTime.year, 1 )) == ' ' )
                        *var = '0';
		...
		case VAR_DAY:
		    ...
		    QueryDateTime( &sysDateTime );
                    if (gaInifile.Day2Digit)
                        IntToAscii2( sysDateTime.day, var );
                    else
...
// check for modification or deletion of existing entry
    if ( gaInifile.HistNoDups && ((env_arg = get_list( histstr, glpHistoryList )) != 0L )) {
        history_end = end_of_env( glpHistoryList );
        // get the start of the alias or variable name
        for ( feptr = env_arg; (( feptr > glpHistoryList ) && ( feptr[-1] != '\0' )); feptr-- )
            ;
        // delete an alias or environment variable
        memmove( feptr, next_env( feptr ), (unsigned int)( history_end - next_env(feptr)) + 1);
    }
