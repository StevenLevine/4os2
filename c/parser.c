...
int getline( int fd, char *line, int nMaxSize, int nEditFlag )
{
    ...
    if ( fd == IN_MEMORY_FILE )
        ...
        if ( ( ( fPipe = QueryIsPipeHandle( fd )) != 0 ) || ( ( fDevice = QueryIsDeviceHandle( fd )) != 0 ) ) {
            ...
            i = 0;
            do {         //  ToDo: FileRead can not read single bytes from wrnd32$
            ...
            line[i] = '\0';
#endif
            //TRACE("bytes_read=%d, char[0]=0x%X, 0x%X, str=%-40s", i, (int) *line, (int) *(line+1), line);
            nMaxSize = i;
    ...
    *line = '\0';
    ...
    if ( i >= 0 ) {
        ...
        if ( fPipe == 0 && fDevice == 0 )       // only do a seek on disk files, not pipes or devices
            if ( fd != IN_MEMORY_FILE )
...
int command( char *line, int fOptions )
{
    ...
    for ( ; ; ) {
        ...
        // 20090330 AB change dir even without trailing '\',
        // if level 2, cd to subdir before checking of valid executable name
        if ( gaInifile.CDWithoutBackslash == 2 ) {
            // check if input command is a directory name and append '\' if yes
            rc = xDosFindFirst( pszStartLine,        /* File pattern - all files     */
            ...
            TRACE("checking for dir on '%s' returned=%d found=%u, hdirFindHandle=%d, QueryIsDeviceHandle=%d", pszStartLine, rc, ulFindCount, hdirFindHandle, QueryIsDeviceHandle(hdirFindHandle) );
            if ( rc == NO_ERROR && strlen(pszStartLine) && pszStartLine[0] != '\\' && (FindBuffer.attrFile & FILE_DIRECTORY) ) {
                // input string matches directory, append '\'
                TRACE("DIR match");
                strcat ( pszStartLine, "\\" );
                line++;
            }
            DosClose(hdirFindHandle);
        }
    ...
    if ( *pszStartLine != '\0' ) {
        ...
        if ( *pszStartLine == '(' ) {
            ...
            if ( ( gpIniptr->UnixPaths ) || ( strnicmp( pszStartLine, "../", 3 ) == 0 ) )
                pszCmdName = ntharg( pszStartLine, NTHARG_SWITCH_CHAR_IS_DATA );
            else
...
static int InternalCommands( char *pszStartLine )
{
    ...
    // check for executable file extention
    pszCmdName = first_arg(pszStartLine);
    p = strchr(pszCmdName, '.');
    if (p != NULL) {
        int j;
    ...
    if ( ( rval == CTRLC ) && ( cv.exception_flag & BREAK_ON_ERROR ) )
        ...
        rval = BATCH_RETURN;
    else if ( ( rval == BATCH_RETURN ) ||
              (( gnInternalErrorLevel = rval ) == ABORT_LINE ) ||
              ( gnInternalErrorLevel == IFF_EXPR_FALSE )  ||
              ( gnInternalErrorLevel == IF_EXPR_FALSE ) ) {
        // clear internal error level flag when rval is an internal notice code
    ...
    *(scan( pszCmdName + 1, "`=", QUOTES + 1 )) = '\0';
    ...
    if ( ( strpbrk( ptr, WILD_CHARS ) == NULL ) && (( ptr = searchpaths( pszCmdName, NULL, TRUE )) != NULL ) ) {
        ...
        if ( get_alias( UNKNOWN_COMMAND ) != 0L ) {
            ...
            // 20090330 AB change dir even without trailing '\'
            // if level 1 after search for external executables)
            if ( gaInifile.CDWithoutBackslash == 1 ) {
                // check if input command is a directory name and append '\' if yes
                rc = xDosFindFirst( pszStartLine,   /* File pattern                  */
                                    &hdirFindHandle,     /* Directory search handle      */
                                    MUST_HAVE_DIRECTORY, /* Search attribute             */
                                    &FindBuffer,         /* Result buffer                */
                                    ulResultBufLen,      /* Result buffer length         */
                ...
                if ( rc == NO_ERROR ) {
                    // input string matches directory
                    // append '\'
                    strcat ( pszStartLine, "\\" );
                    rc = parse_line( "cdd", pszStartLine, cdd_cmd, CMD_STRIP_QUOTES );
                } else {
                    // change directory did not work
            ...
            if ( is_file( szFileName ) ) {
                ...
                for ( n = 0; (( feptr != 0L ) && ( *feptr != '\0' )); feptr = next_env( feptr ) ) {
                    ...
                    if ( *feptr == '.' ) {
                        ...
                        if ( find_file( FIND_FIRST, szFileName, FIND_4OS2_STD, &dir, szFileName ) != NULL )
                            return szFileName;
                    }
                }
            }
