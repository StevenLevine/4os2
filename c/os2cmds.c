...
int memory_cmd( int argc, char **argv )
{
    ...
    printf( TOTAL_OS2_PHYSICAL_MEM, QuerySysInfo(QSV_TOTPHYSMEM) );
    printf( TOTAL_OS2_AVAIL_MEM, QuerySysInfo(QSV_TOTAVAILMEM) );
    printf( TOTAL_OS2_RESIDENT_MEM, QuerySysInfo(QSV_TOTRESMEM));
    ...
    printf( TOTAL_DIRHISTORY, (long)gpIniptr->DirHistorySize );
...
int title_cmd( int argc, char **argv )
{
    ...
    // 20090410 AB added /c switch, ToDo: make this more elegant
    if ( argv[1][0] == '/' || argv[1][0] == '-' ) {
        if ( argv[1][1] == 'd' || argv[1][1] == 'D' ) {
            // set back to directory in title
        ...
        if ( argv[1][1] == 'c' || argv[1][1] == 'C' ) {
            // set back to default title (4os2 window)
            gaInifile.TitleIsCurDir = FALSE;
            // step one, set switch list title
            update_task_list(  ( gpLIS->typeProcess == PT_FULLSCREEN ) ? OS2_FS : OS2_WIN );
            // step two, clear session title and let update_task_list do the rest
            gszSessionTitle[0] = '\0';
            update_task_list( NULL );
            return( 0 );
        }
...
int window_cmd( int argc, char **argv )
{
    ...
    if ( strnicmp( arg, "MAX", 3 ) == 0 ) {
        ...
        if ( sscanf( arg+1, "%60[^\"]", szWindowTitle ) > 0 ) {

            int savedTitleIsCurDir = gaInifile.TitleIsCurDir;
            if ( cv.bn >= 0 ) {
            ...
            update_task_list( NULL );
            gaInifile.TitleIsCurDir = savedTitleIsCurDir;
        }
